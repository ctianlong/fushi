...
public class GifCaptcha {
    ...
    private BufferedImage graphicsImage(Color[] fontcolor, char[] strs, int flag) {
        ...
        for (int i = 0; i < word.length(); i++) {
            ...
            g2d.drawOval(RandomUtil.num(width), RandomUtil.num(height), RandomUtil.num(5,30), 5 + RandomUtil.num(5,30));//绘制椭圆边框
    ...
    protected char[] createWordChar() {
        word = words.get(RandomUtil.num(words.size()));
    ...
    protected Color getRandomColor(int min, int max) {
        ...
        return new Color(min + RandomUtil.num(max - min), min + RandomUtil.num(max - min), min + RandomUtil.num(max - min));
    }
}
